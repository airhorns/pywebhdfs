...
class PyWebHdfsClient(object):
    ...
    def create_file(self, path, file_data, **kwargs):
        ...
        if not init_response.status_code == http.client.TEMPORARY_REDIRECT:
        ...
        if not response.status_code == http.client.CREATED:
...
def _raise_pywebhdfs_exception(resp_code, message=None):
    ...
    if resp_code == http.client.BAD_REQUEST:
        raise errors.BadRequest(msg=message)
    elif resp_code == http.client.UNAUTHORIZED:
        raise errors.Unauthorized(msg=message)
    elif resp_code == http.client.NOT_FOUND:
        raise errors.FileNotFound(msg=message)
    elif resp_code == http.client.METHOD_NOT_ALLOWED:
